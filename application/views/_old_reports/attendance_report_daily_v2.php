<script>
    if (window.top == window) {
        ...
        window.setTimeout('location.reload()', 60000); //reloads after 5 minutes
    } else {
...
<script>
    $(function () {
        $('#txtDateReport').datepicker({
            showOn:"both",
            buttonImage:"<?=base_url('styles/img/calendar.gif')?>",
        ...
        $('#btn_rank_board,#btn_close_rank_best').click(function (e) {
            $('#rank_board').slideToggle('100');
        });
        $('#btn_rank_board_late,#btn_close_rank_worst').click(function (e) {
            $('#rank_board1').slideToggle('100');
        });
    })
    ;
</script>
...
<div class="align_center">
    ...
    <input id="btn_rank_board" type="button" title="Ranking starts from 8:30 AM" value=":) TOP EARLIEST :)">
    <? }?>
    <? if (count($worst_staff) > 0) { ?>
    <input id="btn_rank_board_late" type="button" title="Ranking starts from 8:30 AM" value=":( TOP LATE :(">
    <? }?>
</div>


<? if (count($best_staff) > 0) { ?>
<div id="rank_board" style="background: #0078ae;padding:2px; border: 1px solid;text-align: center" title="Click to hide">
    <img src="<?=base_url('styles/img/x.png')?>" style="float: right" title="Click to hide" id="btn_close_rank_best"/>
    <img src="<?=base_url('styles/img/tip_info.png')?>" style="float: right"
         title="To keep it fair, only submissions taken from 8:30 AM & afterward are eligible for ranking. You can still submit before 8:30 AM but it won't be counted for ranking."/>
    <ul style="display: inline-block;">
        ...
        foreach ($best_staff as $item) {
            ...
            ?>
            <li style="width: 50px;float: left;text-align: center;color: green;background: #fff;padding: 3px">
                <a class="ttest" style="height:75px;display: block;"
                   href="<?=base_url('staff/' . $item->staff_id)?>">
                    <img src="<?=base_url('photos/staff/' . $item->img)?>" width="50px"
                ...
                <p>
                    <!--                    <img src="-->
                        <?//=base_url('styles/img/medal' . $iCount . '.png')?><!--" width="30px" alt=""><br/>-->
...
<? } ?>
<? if (count($worst_staff) > 0) { ?>
<div id="rank_board1" style="border: 1px solid;text-align: center;background: #111;" title="Click to hide">
    <img src="<?=base_url('styles/img/x.png')?>" style="float: right" title="Click to hide" id="btn_close_rank_worst"/>
    <img src="<?=base_url('styles/img/tip_info.png')?>" style="float: right"
         title="Submissions which are taken after 11:00 AM will be considered as late."/>
    <ul style="display: inline-block;">
        <?$iCount = 0;
        foreach ($worst_staff as $item) {
            $iCount++;
            $objSubmitTime = new DateTime($item->updated_on);
            $objSubmitTime = $objSubmitTime->format('H:i:s');
            ?>
            <li style="width: 50px;float: left;text-align: center;color: red;background: #fff;padding: 3px;opacity: 1">
                <a class="ttest" style="height:75px;display: block;"
                   href="<?=base_url('staff/' . $item->staff_id)?>">
                    <img src="<?=base_url('photos/staff/' . $item->img)?>" width="50px"
                         title="<?=$item->updated_by . ' - ' . $objSubmitTime?>">
                ...
                <td class="align_left">
                    <a id="<?=$item->id?>" class="class_name" title="<?=$item->class_name?>" href="">
                        ...
                        if ($objSubmitTime > DEADLINE_ATTENDANCE_SUBMIT) {
                            ...
                            <img class="late_img" src="<?=base_url('styles/img/emoticon_sad.png')?>" height="20px"
                                 title="Late..">
                            <? } else { ?>
                            <?= $item->class_name ?>
                            <? }?>
